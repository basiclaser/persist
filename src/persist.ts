...
function getLocalStoragePersistRelatedData(persistOptions?: PersistOptions): LocalStoragePersistField[] {
        ...
        if (bodyMode || field.hasAttribute("persist") || field.closest("form")?.hasAttribute("persist")) {
            ...
            if (data) {
                ...
                switch (field.tagName.toLowerCase()) {
                    ...
                    case "select":
                        if (field.hasAttribute("multiple")) {
                            const values = data.value as (string | number | boolean)[];
...
export function showPersistManager({ verbose }: PersistOptions) {
